...
namespace LocallySourced.Models
{
    public class LocallySourcedDBInitializer : DropCreateDatabaseAlways<LocallySourcedDB>
    {
         ...
         public static void InitializeIdentityForEF(LocallySourcedDB db)
         {
             ...
             if (user == null)
             {
                 user = new Member { UserName = name, Email = name, NickName = roleName, FirstName = roleName, LastName = roleName, DateJoined = DateTime.Parse("2000-01-01") };
                 var result = userManager.Create(user, password);
                 InitializeAuthenticationApplyAppCookie(userManager, user);
